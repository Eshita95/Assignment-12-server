...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.yfatqfh.mongodb.net/?retryWrites=true&w=majority`;
...
async function run() {
    try {
        ...
        const toolsCollection = client.db("products").collection("product");
        const usersCollection = client.db("products").collection("users");
        const ordersCollection = client.db("products").collection("myOrders");
        const paymentsCollection = client.db("products").collection("payments");
        const reviewsCollection = client.db("products").collection("userReview");
        const queriesCollection = client.db("products").collection("queries");
